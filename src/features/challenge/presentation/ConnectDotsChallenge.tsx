...
import { generateConnectDotsPattern, type ConnectDot } from "../domain/connectDots";
...
const Segment = ({ from, to, color }: { from: ConnectDot; to: ConnectDot; color: string }) => {
...
export function ConnectDotsChallenge({ onAccepted, onSessionTransition }: AlarmChallengeViewProps) {
  ...
  const dots = useMemo<PositionedDot[]>(() => pattern.points.map((point, index) => ({
    index,
    x: point.x * boardMetrics.width,
    y: point.y * boardMetrics.height,
  })), [boardMetrics.height, boardMetrics.width, pattern]);

  const findDot = useCallback((x: number, y: number) => {
    const hitRadius = Math.max(30, Math.min(boardMetrics.width, boardMetrics.height) * 0.09);
    return dots.find((dot) => Math.hypot(dot.x - x, dot.y - y) <= hitRadius)?.index;
  ...
  const toBoardPoint = useCallback((pageX: number, pageY: number): ConnectDot | null => {
  ...
  const confirmNextDot = useCallback((index: number, touchPoint?: ConnectDot) => {
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderGrant: (event, gestureState) => {
      if (gestureState.numberActiveTouches !== 1 || event.nativeEvent.touches.length !== 1) {
        resetActiveSegment();
        return;
      }
      const touchPoint = toBoardPoint(event.nativeEvent.pageX, event.nativeEvent.pageY);
  ...
  const visibleSegments = dots.slice(0, Math.max(0, completedCount)).map((dot, index) =>
    index === 0 ? null : <Segment key={`${index - 1}-${index}`} from={dots[index - 1]!} to={dot} color="#16a34a" />,
  );
  const activeSegment = pointer && completedCount > 0 && completedCount < dots.length
    ? <Segment from={dots[completedCount - 1]!} to={pointer} color="#93c5fd" />
    : null;

  return (
    <View style={styles.card}>
      <Text style={styles.instructions}>Connect the numbered dots in order without lifting through the wrong dot.</Text>
      <Text accessibilityRole="header" style={styles.progress}>Dots connected: {completedCount} of {dots.length}</Text>
      <View
        ref={boardRef}
      ...
      >
        ...
        {dots.map((dot) => {
          ...
          return (
            <Pressable
              key={dot.index}
              onPress={() => confirmNextDot(dot.index)}
              style={[styles.dot, { left: dot.x - dotDiameter / 2, top: dot.y - dotDiameter / 2 }, connected && styles.dotConnected]}
              accessibilityRole="button"
              accessibilityLabel={`Dot ${dot.index + 1}${connected ? ", connected" : ""}`}
              accessibilityHint={`Connect dot ${dot.index + 1} in sequence.`}
            >
...
const styles = StyleSheet.create({
  card: { gap: 14, padding: 16, borderRadius: 16, backgroundColor: "#fff" },
  instructions: { color: "#374151", textAlign: "center" },
  progress: { fontWeight: "700", textAlign: "center" },
  board: { height: 360, borderRadius: 14, backgroundColor: "#eff6ff", borderWidth: 1, borderColor: "#bfdbfe", overflow: "hidden" },
  segment: { position: "absolute", height: 4, borderRadius: 2 },
  dot: { position: "absolute", width: dotDiameter, height: dotDiameter, borderRadius: dotDiameter / 2, backgroundColor: "#1d4ed8", borderWidth: 3, borderColor: "#dbeafe", justifyContent: "center", alignItems: "center" },
  dotConnected: { backgroundColor: "#16a34a", borderColor: "#bbf7d0" },
  dotText: { color: "white", fontWeight: "800" },
  message: { color: "#374151", minHeight: 20, textAlign: "center" },
  retry: { backgroundColor: "#1d4ed8", padding: 14, borderRadius: 10, alignItems: "center" },
