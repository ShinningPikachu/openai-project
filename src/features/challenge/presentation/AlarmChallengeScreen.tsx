...
import { challengeTypeLabels, type Alarm } from "@/features/alarms/domain/alarm";
import { alarmNativeActions } from "@/platform/alarmScheduler";
import { ActiveAlarmMismatchError, completeAcceptedChallenge } from "../application/completeChallenge";
...
import { transitionChallenge, type ChallengeStatus, type ChallengeSession } from "../domain/ChallengeSession";
...
export function AlarmChallengeScreen({ alarm, onCompleted }: { alarm: Alarm; onCompleted(): void }) {
  const [session, setSessionState] = useState<ChallengeSession>(() =>
    startAlarmChallenge(alarm.id, alarm.targetShapeId),
  );
  const [error, setError] = useState<string | null>(null);
  const sessionRef = useRef(session);
  const setSession = useCallback((next: ChallengeSession | ((current: ChallengeSession) => ChallengeSession)) => {
    const resolved = typeof next === "function" ? next(sessionRef.current) : next;
    sessionRef.current = resolved;
    setSessionState(resolved);
  }, []);

  const onSessionTransition = useCallback((status: ChallengeStatus, result?: AlarmChallengeResult) => {
    setSession((current) => transitionChallenge(current, status, result));
  }, [setSession]);

  const onError = useCallback((message: string, cause?: unknown) => {
    if (cause) console.error("Alarm challenge failed.", cause);
    setError(message);
    setSession((current) => transitionChallenge(current, "failed"));
  }, [setSession]);

  const onAccepted = useCallback(async (result: AlarmChallengeResult) => {
    const accepted = transitionChallenge(sessionRef.current, "accepted", result);
    try {
  ...
  }, [onCompleted, onError, setSession]);
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Stop alarm",
          style: "destructive",
          onPress: () => void alarmNativeActions
  ...
  const challengeProps = { alarm, session, onSessionTransition, onAccepted, onRetry, onError };

  return (
    <View style={styles.container}>
      <Text style={styles.label}>{alarm.label || "Alarm"}</Text>
      <Text style={styles.type}>{challengeTypeLabels[alarm.challengeType]}</Text>
      {alarm.challengeType === "shape-photo" ? <CameraChallengeScreen {...challengeProps} /> : null}
      {alarm.challengeType === "quick-addition" ? <QuickAdditionChallenge {...challengeProps} /> : null}
      {alarm.challengeType === "connect-dots" ? <ConnectDotsChallenge {...challengeProps} /> : null}
...
const styles = StyleSheet.create({
  ...
  type: { color: "#4b5563", fontSize: 16, fontWeight: "700", textAlign: "center" },
