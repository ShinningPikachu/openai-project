...
export class ExactAlarmPermissionError extends Error { name = "ExactAlarmPermissionError"; }
export class AlarmSchedulingError extends Error { name = "AlarmSchedulingError"; constructor(readonly alarmId: string, cause: unknown) { super(`Could not schedule alarm ${alarmId}: ${String(cause)}`); } }
...
const native = (): NativeModule => { const module = reactNative().NativeModules?.ShapeAlarmScheduler as NativeModule | undefined; if (!module) throw new AlarmServiceUnavailableError("Native alarm scheduler is unavailable in Expo Go or this platform."); return module; };
...
export class NativeAlarmScheduler implements AlarmScheduler { async schedule(alarm: Alarm) { if (!isAndroid()) return; const payload = toNativeAlarmPayload(alarm); if (!payload) { await this.cancel(alarm.id); return; } await native().scheduleAlarm(payload); } async cancel(alarmId: string) { if (isAndroid()) await native().cancelAlarm(alarmId); } async rescheduleAll(alarms: Alarm[]) { if (!isAndroid()) return; await native().cancelAllAlarms(); for (const alarm of alarms) if (alarm.enabled) await this.schedule(alarm); } requestPermissions() { return isAndroid() ? native().requestNotificationPermission() : Promise.resolve(nonAndroidPermissionStatus); } getPermissionStatus() { return isAndroid() ? native().getAlarmPermissionStatus() : Promise.resolve(nonAndroidPermissionStatus); } }
export const alarmNativeActions = { openExactAlarmSettings: () => native().openExactAlarmSettings(), openNotificationSettings: () => native().openNotificationSettings(), getActiveAlarm: () => native().getActiveAlarm(), getScheduledAlarms: () => native().getScheduledAlarms(), stopActiveAlarm: (alarmId: string, reason: "long-press" | "emergency-override") => native().stopActiveAlarm(alarmId, reason) };
